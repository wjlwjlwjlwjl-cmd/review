...
3. 普通信号，对于多次接受的情况，只记录一次；但是对于实时信号，会通过添加到sigqueue链表的方式多次记录
...
## 时钟中断
上面理解了操作系统如何既进行用户代码的处理也处理硬件中断，那进程调度呢？现代cpu中集成了一个时钟源，其每个一段时间就会向cpu中发送时钟中断。cpu获得
...
## 内核态和用户态
以32位机器为例，在进程的地址空间当中[0，3]GB是用户的代码（代码段、数据段、堆、栈、共享区等），[3, 4]GB是内核的代码。运行用户的代码时，就是用户态；运行内核的代码时，就是内核态
...
多级页表对于内存的连续性降低了要求，但是查询页表的时间也是额外的损耗。所以引入了TLB快表，是MMU内部的一块高速缓存。如果内部
已经缓存了虚拟地址到物理地址的映射信息，那么就不需要再查询页表了，否则引起缺页中断

## 缺页中断
根据是否加载进了内存和方位情况，缺页中断可以分为三种类型：

1. 硬缺页：物理内存中并没有加载进物理页，所以去磁盘上读取拷贝到物理内存，建立映射关系
2. 软缺页：物理内存中已经加载进了物理页，但是当前进程并不知道（映射关系没有更新），那么久不会从磁盘再次读取，而是直接更新页表
3. 无效缺页错误：越界访问、空指针解引用等
### 如何区分越界访问和缺页
1. 页号合法性检查
* 如果页号合法但页面不在内存中，则缺页；如果页号非法，则越界访问
2. 内存映射检查
* 如果虚拟地址在当前进程的映射范围中，那么缺页，否则越界访问

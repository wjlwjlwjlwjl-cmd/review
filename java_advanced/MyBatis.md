...
它和 Mybatis 的关系就像 C++ 和 C 的关系。

* mapper 自动生成部分接口 `public interface UserInfoMapper extends BaseMapper<UserInfo>`

* `@tableName` 和 `@tableField` 可以解决非小驼峰命名导致无法转换的问题

* `@tableId` 用来映射数据库主键列

* 条件构造器

  * 条件构造器允许在不通过注解、xml的方式就方便地构造查询

  * 主要的 Wrapper 类 `QueryWrapper`、`UpdateWrapper`、`LambdaQueryWrapper`、`LambdaUpdateWrapper`，两个 Lambda 主要是为了解决硬编码导致的手误问题，导致查询出错，大体使用如下

  ```java
        QueryWrapper<UserInfo> queryWrapper = new QueryWrapper<UserInfo>()
                .select("username", "age", "password")
                .eq("username", "wangjiale")
                .orderByAsc("age");
        List<UserInfo> list = mapper.selectList(queryWrapper);

        UpdateWrapper<UserInfo> updateWrapper = new UpdateWrapper<UserInfo>()
                .set("username", "intellij")
                .in("id", List.of(1));
        mapper.update(updateWrapper);

        LambdaQueryWrapper<UserInfo> queryWrapper = new LambdaQueryWrapper<UserInfo>()
                .select(UserInfo::getUsername, UserInfo::getAge, UserInfo::getGender)
                .eq(UserInfo::getId, 2);
        List<UserInfo> rets = mapper.selectList(queryWrapper);
  ```
